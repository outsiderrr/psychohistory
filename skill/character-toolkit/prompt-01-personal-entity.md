...
**这不是简单的复制粘贴**。女娲的输出是叙事式的散文，references.md 要把它重新编排成有明确 §1~§9 章节、每条结论都带证据引用和 strength 标注的研究文档。

### 目标结构

写入以下 9 个章节（编号和 Appendix Research Hand-off Template + Phase 4 产出模板完全一致，1:1 对齐）：
...
- **§6 Expression DNA** — 修辞风格、签名用语、确定性表达模式
- **§7 Values Hierarchy** — 核心价值观的优先级排序及冲突取舍
- **§8 Known Biases — Basis** — 每个偏差的识别方式和例子
- **§9 Honesty Boundaries — Rationale** — 每条边界的来由
...
**Checkpoint 2**：`references.md` 包含 §1~§9 的完整研究笔记。
...
- [ ] JSON 可通过标准 JSON 解析器
- [ ] references.md 的 §1~§9 每节都有实质内容，不是空壳
